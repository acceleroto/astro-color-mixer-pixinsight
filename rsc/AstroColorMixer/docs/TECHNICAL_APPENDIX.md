...
This appendix describes the processing model used by Astro Color Mixer and the practical assumptions behind it. The tool is designed for nonlinear RGB astrophotography images and combines hue-band selection, luminance-range masking, low-saturation handling, and sequential refinement passes.
...
- a non-destructive output-to-new-image workflow
...
- final Apply to New Image uses full-resolution source data
...
Final Apply to New Image operates on the full-resolution source image, which is why small local detail differences can exist even when the overall preview direction is accurate.
